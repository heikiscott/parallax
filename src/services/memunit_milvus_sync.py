...
from retrieval.services.vectorize import DeepInfraVectorizeServiceInterface
...
class MemUnitMilvusSyncService:
    ...
    def __init__(
        ...
        logger.info("MemUnitMilvusSyncService 初始化完成（仅同步 episode 到 Milvus + ES）")
...
def get_memunit_milvus_sync_service() -> MemUnitMilvusSyncService:
    """获取 MemUnit Milvus 同步服务实例
    
    通过依赖注入框架获取服务实例，支持单例模式。
    """
    from core.di import get_bean
    return get_bean("memunit_milvus_sync_service")
