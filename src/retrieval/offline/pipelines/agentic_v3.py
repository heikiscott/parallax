"""Agentic Retrieval V3 - Pure ColBERT with Type-Aware Multi-Query.

This module implements ColBERT-based agentic retrieval, replacing the hybrid
search approach from V2 with pure ColBERT late interaction retrieval.

Key differences from V2:
- Uses ColBERT MaxSim instead of Embedding + BM25 + RRF
- No separate reranker step (ColBERT serves as retriever + reranker)
- No cluster expansion (ColBERT's token matching reduces the need)
- More conservative parameters (ColBERT precision is higher)
...
Question Classification
  ...
  │         │
  │         ▼
  │    Single Query ColBERT → Top 10 → Sufficiency Check
  │
  └─── Complex (EVENT_*, COUNTING, REASONING_*)
            │
            ▼
       Type-Aware Multi-Query (2-3) → Parallel ColBERT → RRF Fusion
            │
            ▼
       Sufficiency Check
            │
            ...
            └─── Insufficient → Round 2 (missing_info queries)
                      │
                      ▼
                 ColBERT Search → Merge → Return Final
```

Design Philosophy:
- Keep V2's agentic flow (classification, multi-query, sufficiency check)
- Replace hybrid search with pure ColBERT
- Remove Reranker (ColBERT's MaxSim is the ranking)
- Remove Cluster Expansion (simplify flow)
- Use more conservative parameters (ColBERT is more precise)
"""
...
def _get_v3_config(config: Any, key: str, default: Any) -> Any:
    """Get V3-specific config value from config.retrieval.agentic_v3.xxx."""
    retrieval_cfg = getattr(config, 'retrieval', None)
    if retrieval_cfg is None:
        return default
    v3_cfg = getattr(retrieval_cfg, 'agentic_v3', None)
    if v3_cfg is None:
        return default
    return getattr(v3_cfg, key, default)


def _get_type_retrieval_config_v3(config: Any, question_type: QuestionType) -> Optional[dict]:
    """Get type-specific retrieval config for V3.

    Looks up config.retrieval.v3_type_retrieval_configs.{type_name}
    ...
    v3_type_configs = getattr(retrieval_cfg, 'v3_type_retrieval_configs', None)
    if v3_type_configs is None:
        return None

    # Try to get config for this specific type
    type_key = question_type.value
    type_config = getattr(v3_type_configs, type_key, None)

    # Fallback to default config
    if type_config is None:
        type_config = getattr(v3_type_configs, 'default', None)
...
# =============================================================================
# Type-Aware Multi-Query Generation (reused from V2)
# =============================================================================
...
# =============================================================================
# Main Entry Point: agentic_retrieval_v3
# =============================================================================
...
async def agentic_retrieval_v3(
    ...
    Key difference from V2: Uses ColBERT instead of hybrid search.
    ColBERT serves as both retriever and reranker (no separate rerank step).
    ...
    Args:
        ...
        llm_provider: LLM Provider instance
        llm_config: LLM configuration dict
        colbert_index: Pre-built ColBERT index with document embeddings
            Format: [{"doc": {...}, "embeddings": np.ndarray}, ...]
    ...
    logger.info(f"{'='*60}")
    logger.info(f"Agentic Retrieval V3 (ColBERT): {query[:60]}...")
    logger.info(f"{'='*60}")
    ...
    if type_config:
        ...
        # Fallback to agentic_v3 default config
        round1_top_n = _get_v3_config(config, 'round1_top_n', 12)
        round2_top_n = _get_v3_config(config, 'round2_top_n', 15)
        merge_budget = _get_v3_config(config, 'merge_budget', 20)
        final_top_n = _get_v3_config(config, 'final_top_n', 12)

        logger.info(f"  [TypeConfig] Using default config (v3_type_retrieval_configs not found)")

    num_queries = _get_v3_config(config, 'num_queries', 3)
    confidence_threshold = _get_v3_config(config, 'confidence_threshold', 0.85)
    rrf_k = _get_v3_config(config, 'rrf_k', 60)
    ...
    # ========== Step 4: Round 1 ColBERT Retrieval ==========
    if use_mq_round1:
        # === Multi-Query Path ===
        ...
        logger.info(f"  [Round 1] Executing {len(queries)} ColBERT queries in parallel...")

        # Parallel ColBERT search for each query
        search_tasks = [
            colbert_search(
                query=q,
        ...
        round1_results = multi_colbert_rrf_fusion(multi_results, k=rrf_k)
        ...
        # === Single Query Path ===
        logger.info(f"  [Round 1] Single query ColBERT search...")

        if enable_traversal_stats:
            round1_results, r1_stats = await colbert_search(
                query=query,
                colbert_index=colbert_index,
                top_n=round1_top_n,
            ...
            round1_results = await colbert_search(
                query=query,
                colbert_index=colbert_index,
                top_n=round1_top_n,
            )
    ...
    logger.info(f"  [Round 1] Retrieved {len(round1_results)} documents via ColBERT")
    ...
    # Note: No separate reranking - ColBERT results are already well-ranked
    ...
    if is_sufficient:
        logger.info(f"  [Decision] Sufficient! Returning Round 1 ColBERT results")
    ...
    # Parallel ColBERT retrieval for Round 2 queries
    logger.info(f"  [Round 2] Executing {len(refined_queries)} ColBERT queries...")
    ...
    round2_results = multi_colbert_rrf_fusion(round2_multi_results, k=rrf_k)
    ...
    # Final selection (ColBERT scores are already quality-ranked, no need for reranking)
