...
async def run_search_stage_v3(
    ...
    """
    V3 专用检索阶段 - 纯 ColBERT 检索

    与 run_search_stage 完全独立，调用 adapter.search_v3()

    Args:
        adapter: 系统适配器 (需要实现 search_v3 方法)
        qa_pairs: QA 对列表
        index: 索引信息
        conversations: 对话列表
        checkpoint_manager: 断点续传管理器
    ...
    """
    logger.info(f"\n{'='*60}")
    logger.info(f"🚀 Search stage V3 (Pure ColBERT)")
    logger.info(f"{'='*60}")
    ...
    pbar = tqdm(
        total=total_questions,
        initial=processed_questions,
        desc="🔍 V3 ColBERT Search",
        unit="qa"
    )

    async def search_single_v3(qa):
        """调用 adapter.search_v3()"""
        set_activity_id(f"search-v3-{qa.question_id}")

        async with semaphore:
            conv_id = qa.metadata.get("conversation_id", "0")
            conversation = conv_id_to_conv.get(conv_id)
            # 🔥 关键：调用 search_v3 而不是 search
            result = await adapter.search_v3(qa.question, conv_id, index, conversation=conversation)
            pbar.update(1)
            return result
    ...
    for idx, (conv_id, qa_list) in enumerate(sorted(conv_to_qa.items())):
        ...
        # 并发处理这个会话的所有问题
        tasks = [search_single_v3(qa) for qa in qa_list]
        results_for_conv = await asyncio.gather(*tasks)
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"🎉 V3 ColBERT Search completed!")
    logger.info(f"{'='*60}")
