"""Search Stage V4 - Cross-Attention Retrieval.

This module implements the V4 search stage using cross-attention retrieval.
Completely independent from V3 (ColBERT) implementation.
"""
...
async def run_search_stage_v4(
    ...
    与 run_search_stage_v3 完全独立，调用 adapter.search_v4()

    Args:
        adapter: 系统适配器 (需要实现 search_v4 方法)
        qa_pairs: QA 对列表
        index: 索引信息
        conversations: 对话列表
    ...
    """
    logger.info(f"\n{'='*60}")
    logger.info(f"🚀 Search stage V4 (Cross-Attention)")
    logger.info(f"{'='*60}")
    ...
    # 构建 conversation_id 到 conversation 的映射
    ...
    pbar = tqdm(
        ...
        desc="🔍 V4 Cross-Attention Search",
        unit="qa"
    )

    async def search_single_v4(qa):
        """调用 adapter.search_v4()"""
        set_activity_id(f"search-v4-{qa.question_id}")
    ...
    for idx, (conv_id, qa_list) in enumerate(sorted(conv_to_qa.items())):
        ...
        # 并发处理这个会话的所有问题
        tasks = [search_single_v4(qa) for qa in qa_list]
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"🎉 V4 Cross-Attention Search completed!")
    logger.info(f"{'='*60}")
